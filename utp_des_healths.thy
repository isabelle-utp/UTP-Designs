...
[pred, rel]: "H1(P) = ((ok\<^sup><)\<^sub>u \<Rightarrow> P)"
...
theorem H1_algebraic_intro:
  assumes
    "(true\<^sub>h \<Zcomp> R) = true\<^sub>h"
    "(II\<^sub>D \<Zcomp> R) = R"
  shows "R is H1"
proof -
  have "R = (II\<^sub>D \<Zcomp> R)" by (simp add: assms(2))
  also have "... = (H1(II) \<Zcomp> R)"
    by (simp add: H1_design_skip)
  also have "... = (((ok\<^sup><)\<^sub>u \<Rightarrow> II) \<Zcomp> R)"
    by (simp add: H1_def)
  also have "... = (((\<not> (ok\<^sup><)\<^sub>u) \<Zcomp> R) \<or> R)"
    by (simp add: impl_alt_def seqr_or_distl)
  also have "... = ((((\<not> (ok\<^sup><)\<^sub>u) \<Zcomp> true\<^sub>h) \<Zcomp> R) \<or> R)"
    by (pred_auto)
  also have "... = (((\<not> (ok\<^sup><)\<^sub>u) \<Zcomp> true\<^sub>h) \<or> R)"
    by (metis assms(1) seqr_assoc)
  also have "... = ((ok\<^sup><)\<^sub>u \<Rightarrow> R)"
    by (pred_auto)
  finally show ?thesis by (metis H1_def Healthy_def')
qed

lemma nok_not_false:
  "(\<not> (ok\<^sup><)\<^sub>u) \<noteq> false"
  by (pred_auto)
...
theorem H1_left_zero:
  ...
  shows "(true ;; P) = true"
proof -
  from assms have "(true ;; P) = (true ;; ((ok\<^sup><)\<^sub>u \<Rightarrow> P))"
    by (simp add: H1_def Healthy_def')
  (* The next step ensures we get the right alphabet for true by copying it *)
  also from assms have "... = (true ;; (\<not> (ok\<^sup><)\<^sub>u \<or> P))" (is "_ = (?true ;; _)")
    by (simp add: impl_alt_def)
  also from assms have "... = ((?true ;; (\<not> (ok\<^sup><)\<^sub>u)) \<or> (?true ;; P))"
    by pred_auto
...
theorem H1_left_unit:
  ...
  shows "(II\<^sub>D ;; P) = P"
proof -
  have "(II\<^sub>D ;; P) = (((ok\<^sup><)\<^sub>u \<Rightarrow> II) ;; P)"
    by (metis H1_def H1_design_skip)
  also have "... = (((\<not> (ok\<^sup><)\<^sub>u) ;; P) \<or> P)"
    by (simp add: impl_alt_def seqr_or_distl)
  also from assms have "... = ((((\<not> (ok\<^sup><)\<^sub>u) ;; true\<^sub>h) ;; P) \<or> P)"
    by (rel_auto)
  also have "... = (((\<not> (ok\<^sup><)\<^sub>u) ;; (true\<^sub>h ;; P)) \<or> P)"
    by (simp add: seqr_assoc)
  also from assms have "... = ((ok\<^sup><)\<^sub>u \<Rightarrow> P)"
  proof -
    have a:"((\<not> (ok\<^sup><)\<^sub>u) ;; (true\<^sub>h ;; P)) = ((\<not> (ok\<^sup><)\<^sub>u) ;; true\<^sub>h)"
      using assms 
      by (simp add: H1_left_zero, rel_simp)
    also have b:"... = (\<not> (ok\<^sup><)\<^sub>u)"
      by rel_auto
    finally show ?thesis
      by (simp add:a b impl_alt_def)
  qed
...
theorem H1_nok_left_zero:
  ...
  assumes "P is H1"
  shows "((\<not> (ok\<^sup><)\<^sub>u) ;; P) = (\<not> (ok\<^sup><)\<^sub>u)"
proof -
  have "((\<not> (ok\<^sup><)\<^sub>u) ;; P) = (((\<not> (ok\<^sup><)\<^sub>u) ;; true\<^sub>h) ;; P)"
    by rel_auto
  also have "... = ((\<not> (ok\<^sup><)\<^sub>u) ;; true\<^sub>h)"
    by (metis H1_left_zero assms seqr_assoc)
  also have "... = (\<not> (ok\<^sup><)\<^sub>u)"
    by rel_auto
  finally show ?thesis .
qed
...
lemma H1_choice_closed [closure]:
  ...
  by (simp add: H1_def Healthy_def' impl_alt_def disj_pred_def semilattice_sup_class.sup_left_commute)
...
(*term "A \<and> II \<down> more\<^sub>L\<^sup>2"
term "II"
term "ok\<^sup>>"
term "(ok\<^sup>> = false)\<^sub>u"
term "(ok\<^sup>>)\<^sub>u = (false)\<^sub>u"
term "(\<exists>ok\<^sup>> \<Zspot> \<lbrakk>P\<rbrakk>\<^sub>P \<and> \<not>ok\<^sup>>)\<^sub>u"
term "((P \<and> \<not> (ok\<^sup>>)\<^sub>u) ;; II \<down> more\<^sub>L\<^sup>2)"
declare [[show_types]]
*)

definition J :: "'\<alpha> des_hrel" where 
[rel]: "J = (((ok\<^sup><)\<^sub>u \<Rightarrow> (ok\<^sup>>)\<^sub>u) \<and> II \<up> more\<^sub>L\<^sup>2)"

definition H2 where
[rel]: "H2 (P) \<equiv> P ;; J"

lemma J_split:
  shows "(P ;; J) = (P\<^sup>f \<or> (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u))"
proof -
  have "(P ;; J) = (P ;; (((ok\<^sup><)\<^sub>u \<Rightarrow> (ok\<^sup>>)\<^sub>u) \<and> II \<up> more\<^sub>L\<^sup>2))"
    by (simp add: H2_def J_def design_def)
  also have "... = (P ;; (((ok\<^sup><)\<^sub>u \<Rightarrow> (ok\<^sup>< \<and> ok\<^sup>>)\<^sub>u) \<and> II \<up> more\<^sub>L\<^sup>2))"
    by (rel_auto)
  also have "... = ((P ;; (\<not> (ok\<^sup><)\<^sub>u \<and> II \<up> more\<^sub>L\<^sup>2)) \<or> (P ;; ((ok\<^sup><)\<^sub>u \<and> (II \<up> more\<^sub>L\<^sup>2 \<and> (ok\<^sup>>)\<^sub>u))))"
    by (rel_auto)
  also have "... = (P\<^sup>f \<or> (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u))"
  proof -
    have "(P ;; (\<not> (ok\<^sup><)\<^sub>u \<and> II \<up> more\<^sub>L\<^sup>2)) = P\<^sup>f"
    proof -
      have "(P ;; (\<not> (ok\<^sup><)\<^sub>u \<and> II \<up> more\<^sub>L\<^sup>2)) = ((P \<and> \<not> (ok\<^sup>>)\<^sub>u) ;; II \<up> more\<^sub>L\<^sup>2)"
        by (rel_auto)
      also have "... = (\<exists>ok\<^sup>> \<Zspot> \<lbrakk>P\<rbrakk>\<^sub>P \<and> ok\<^sup>> = false)\<^sub>u"
        by rel_auto
      also have "... = P\<^sup>f"
        by rel_auto
      finally show ?thesis .
    qed
    moreover have "(P ;; ((ok\<^sup><)\<^sub>u \<and> (II \<up> more\<^sub>L\<^sup>2 \<and> (ok\<^sup>>)\<^sub>u))) = (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u)"
    proof -
      have "(P ;; ((ok\<^sup><)\<^sub>u \<and> (II \<up> more\<^sub>L\<^sup>2 \<and> (ok\<^sup>>)\<^sub>u))) = (P ;; ((ok\<^sup><)\<^sub>u \<and> II))"
        by (rel_auto)
      also have "... = (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u)"
        by (rel_auto)
      finally show ?thesis .
    qed
...
lemma H2_split:
  shows "H2(P) = (P\<^sup>f \<or> (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u))"
  by (simp add: H2_def J_split)

theorem H2_equivalence:
  "P is H2 \<longleftrightarrow> `\<lbrakk>P\<^sup>f \<Rightarrow> P\<^sup>t\<rbrakk>\<^sub>P`"
proof -
  have "`\<lbrakk>P \<Leftrightarrow> (P ;; J)\<rbrakk>\<^sub>P` \<longleftrightarrow> `\<lbrakk>P \<Leftrightarrow> (P\<^sup>f \<or> (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u))\<rbrakk>\<^sub>P`"
    by (simp add: J_split)
  ...
  also have "... = `\<lbrakk>P\<^sup>t \<Leftrightarrow> (P\<^sup>f \<or> P\<^sup>t)\<rbrakk>\<^sub>P`"
    by (pred_auto)
  also have "... = `\<lbrakk>P\<^sup>f \<Rightarrow> P\<^sup>t\<rbrakk>\<^sub>P`"
    by (pred_auto)
  finally show ?thesis
...
lemma H2_equiv:
  ...
  by (simp add: H2_equivalence pred_impl pred_ref_iff)
...
theorem H2_not_okay: "H2 ((\<not>ok\<^sup><)\<^sub>u) = (\<not>ok\<^sup><)\<^sub>u"
proof -
  have "H2 ((\<not>ok\<^sup><)\<^sub>u) = (((\<not>ok\<^sup><)\<^sub>u)\<^sup>f \<or> (((\<not>ok\<^sup><)\<^sub>u)\<^sup>t \<and> (ok\<^sup>>)\<^sub>u))"
    by (simp add: H2_split)
  also have "... = (\<not> ok\<^sup>< \<or> (\<not>ok\<^sup><) \<and> ok\<^sup>>)\<^sub>u"
    by rel_auto
  also have "... = (\<not> ok\<^sup><)\<^sub>u"
    by (pred_auto)
  finally show ?thesis .
...
proof -
  have "P \<squnion> Q = (P\<^sup>f \<or> P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u) \<squnion> (Q\<^sup>f \<or> Q\<^sup>t \<and> (ok\<^sup>>)\<^sub>u)"
...
theorem H1_H2_commute:
  "H1 (H2 P) = H2 (H1 P)"
proof -
  have "H2 (H1 P) = (((ok\<^sup><)\<^sub>u \<Rightarrow> P) ;; J)"
    by (simp add: H1_def H2_def)
  also have "... = ((\<not> (ok\<^sup><)\<^sub>u \<or> P) ;; J)"
    by (rel_auto)
  also have "... = (((\<not> (ok\<^sup><)\<^sub>u) ;; J) \<or> (P ;; J))"
    by rel_auto
  also have "... =  ((H2 (\<not> (ok\<^sup><)\<^sub>u)) \<or> H2(P))"
    by (simp add: H2_def)
  also have "... =  ((\<not> (ok\<^sup><)\<^sub>u) \<or> H2(P))"
    by rel_auto
  also have "... = H1(H2(P))"
    by (rel_auto)
...
proof -
  have "\<^bold>H(P) = ((ok\<^sup><)\<^sub>u \<Rightarrow> H2(P))"
    by (simp add: H1_def)
  also have "... = ((ok\<^sup><)\<^sub>u \<Rightarrow> (P\<^sup>f \<or> (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u)))"
    by (metis H2_split)
  also have "... = ((ok\<^sup><)\<^sub>u \<and> (\<not> P\<^sup>f) \<Rightarrow> (ok\<^sup>>)\<^sub>u \<and> (ok\<^sup><)\<^sub>u \<and> P\<^sup>t)"
    by (rel_auto)
...
lemma ok_post: "((ok\<^sup>>)\<^sub>u \<and> (post\<^sub>D(P) \<up> more\<^sub>L\<^sup>2)) = ((ok\<^sup>>)\<^sub>u \<and> (P\<^sup>t))"
...
proof -
  ...
  also have "... = ((ok\<^sup><)\<^sub>u \<Rightarrow> (P\<^sup>f \<or> (P\<^sup>t \<and> (ok\<^sup>>)\<^sub>u)))"
    by (metis H2_split)
  also have "... = ((ok\<^sup><)\<^sub>u \<and> (\<not> P\<^sup>f) \<Rightarrow> (ok\<^sup>>)\<^sub>u \<and> P\<^sup>t)"
    by (pred_auto)
  also have "... = ((ok\<^sup><)\<^sub>u \<and> (\<not> P\<^sup>f) \<Rightarrow> (ok\<^sup>>)\<^sub>u \<and> (ok\<^sup><)\<^sub>u \<and> P\<^sup>t)"
    by (pred_auto)
  also have "... = ((ok\<^sup><)\<^sub>u \<and> (pre\<^sub>D(P) \<up> more\<^sub>L\<^sup>2) \<Rightarrow> (ok\<^sup>>)\<^sub>u \<and> (ok\<^sup><)\<^sub>u \<and> (post\<^sub>D(P) \<up> more\<^sub>L\<^sup>2))"
    apply pred_auto
    by (simp add: ok_post ok_pre)
  also have "... = ((ok\<^sup><)\<^sub>u \<and> (pre\<^sub>D(P) \<up> more\<^sub>L\<^sup>2) \<Rightarrow> (ok\<^sup>>)\<^sub>u \<and> (post\<^sub>D(P)\<up> more\<^sub>L\<^sup>2))"
    by (pred_auto)
...
lemma top_d_is_H1_H2 [closure]: "\<top>\<^sub>D is \<^bold>H"
  by (simp add: H1_def H2_not_okay Healthy_intro top_d_def impl_alt_def)
